...
Public Class CortesCentralBascula
    ...
    Private wsCB As wsCentralbascula.Central
    Private DTRespuesta As DataTable
    Private CiaCB As String = ""
    Private CentroOperCB As String = ""
    ...
    Public Sub CortesControlBascula_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            wsCB = New wsCentralbascula.Central
            wsCB.Timeout = 30000
            wsCB.Url = "http://172.31.79.161:8001/Central.svc"

            DTiqDet = New DataTable
            ...
            CiaCB = ConfigParametros("COMPAÑIACB")
            CentroOperCB = ConfigParametros("CENTROOPERCB")
            ...
            'DTiqDet = wsCB.lotes_Produccion("1", "003", Sentencia, Val(ConfigParametros("ULTCONTADORCB")), True)
            FRefrescaDG_Click(Nothing, Nothing)

            'AsignaDataGrid(DGCortesNept, DTiqDet)

            Campos = {"ARTCLO@Cod.Mat", "LTE@Lote", "CNCPTO@Concpto"}
            Campos = AsignaItemsCB(Campos, CBBuscar.ComboBox, DTiqDet)
            FormLoad = True
    ...
    Private Sub FRefrescaDG_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FRefrescaDG.Click
        Try
            ' DTiqDet.Open("Select * from tqte_dtlle where CNCPTO IN (" + Sentencia + ") and (td_rowid>" + ConfigParametros("ULTCONTADORCB") + ") order by td_rowid")
            DTiqDet = wsCB.lotes_Produccion(CiaCB, CentroOperCB, Sentencia, Val(ConfigParametros("ULTCONTADORCB")), True)
            AsignaDataGrid(DGCortesNept, DTiqDet)
        Catch ex As System.ArgumentException
            Evento(ex.ToString)
            TError.Text += vbCrLf + FechaC() + "  " + ex.Message
    ...
    Private Sub TBuscar_KeyUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles TBuscar.KeyUp
        Try
            ...
            BusquedaDG(DGCortesNept, DTiqDet, TBuscar.Text, Campos(x), Hallado)
    ...
    Private Sub TimSeg_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TimSeg.Tick
        Try
            ...
            If (ServComM = True Or UsuarioPrincipal = "TECNIMATICA") AndAlso Cont Mod 60 = 0 Then
    ...
    Private Sub FImport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles FImport.Click
        Try
            ...
            For Each Recordset As DataRow In DTiqDet.Rows
                ...
                DConceptos.Find("CONCEPTO = '" + Recordset("ti_cdgo_cncpto_psje") + "'")

                Placa = Recordset("PLCA")
                Consecutivo = Recordset("ti_cnsctvo")
                CantidadKg = Recordset("KILOS")
                ...
                If TipoOperacion = "ENTRADA" Or TipoOperacion = "SALIDA" Then
                    ...
                    If IsDBNull(Recordset("ARTCLO")) Then Continue For
                    ...
                    If DCortesMP.Count > 0 Then
                        ...
                        DOBsCortesMP.RecordSet("CANTIDAD") = CantidadKg
                    ...
                    Else
                        ...
                        DCortesMP.RecordSet("INVINI") = CantidadKg
                        DCortesMP.RecordSet("LOTE") = Lote
                        DCortesMP.RecordSet("ALARMA") = Math.Round(CantidadKg * 0.01, 2)
                ...
                Else
                    ...
                    DEmpaque.RecordSet("PESO") = CantidadKg
                    ...
                    Evento("Crea registro automático GRANEL tabla EMPAQUE OP: " + DOPs.RecordSet("OP").ToString + "' and PESO: " +
                           CantidadKg.ToString + " Kg PLACA: " + Recordset("PLCA") + " CONTADOR CB:" + Td_RowId_CB)
                    Continue For
                End If
                ...
                If TipoOperacion = "ENTRADA" Then
                    ...
                    DMovEntSal.RecordSet("PLACA") = Placa
                    DMovEntSal.RecordSet("FECHA") = Now.ToString("yyyy/MM/dd HH:mm:ss")
                    DMovEntSal.RecordSet("CANTIDAD") = CantidadKg
                    ...
                    If Not IsDBNull(Recordset("TRCRO")) AndAlso Recordset("TRCRO") <> "" Then
                        DMovEntSal.RecordSet("CODTERCERO") = CLeft(Recordset("TRCRO"), 15)
                        DMovEntSal.RecordSet("NOMTERCERO") = CLeft(Recordset("TE_NMBRE"), 30)
                    End If
